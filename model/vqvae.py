...
class VQAutoencoder(nn.Module):
    def __init__(self, config):
        ...
        self.encode_down_project = nn.Linear(config.embedding_size, config.hidden_size)
...
class VQVAE(LlamaPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.embed_tokens = nn.Embedding(config.vocab_size, config.embedding_size, padding_idx=self.padding_idx)
    ...
    def forward(self, input_ids,attention_mask, labels,question=None):
        ...
        # VQ objective
        loss_vq = F.mse_loss(latents_masked, feature_masked.detach())
        # Commitment objective
        loss_commit = F.mse_loss(feature_masked, latents_masked.detach())
